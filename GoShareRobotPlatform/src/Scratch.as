...
package {
    ...
    public class Scratch extends Application {
        ...
        protected function init():void {
            ...
            stage.addEventListener(Event.ENTER_FRAME, step);///进入帧？
...
//        if (autostart) runtime.startGreenFlags(true);
        ...
        protected function updateLayout(w:int, h:int):void {
            topBarPart.x = 0;
            topBarPart.y = 0;
            topBarPart.setWidthHeight(w, 36);
            ...
            if (editMode) {
                if (stageIsContracted) {
                    stagePart.setWidthHeight(240, 180, 0.5);
                } else {
                ...
                var scale:Number = Math.min((w - pad) / 480, (h - pad) / 360);
                scale = Math.max(0.01, scale);
                var scaledW:int = Math.floor((scale * 480) / 4) * 4; // round down to a multiple of 4
                scale = scaledW / 480;
                presentationScale = scale;
                var playerW:Number = (scale * 480);
                var playerH:Number = (scale * 360);
                stagePart.setWidthHeight(playerW, playerH, scale);

                // TODO
                // 播放器固定的比例为4:3, 无论播放器的高宽比例为多少，都无法在不失真的情况下兼容所有的屏幕尺寸
                // 目前为了达到在所有机器的全屏效果，只能进行缩放处理，缩放可能会导致失真，字体变形等问题，请了解
