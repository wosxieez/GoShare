...
public class BlockMenus implements DragClient {
	...
	public static function BlockMenuHandler(evt:MouseEvent, block:Block, blockArg:BlockArg = null, menuName:String = null):void {
		...
		if (menuName == 'expression') menuHandler.expressionMenu(evt);
...
//			handler.scrollAlignMenu(evt);
	...
	private function expressionMenu(evt:MouseEvent):void
	{
		var m:Menu = new Menu(setBlockArg, 'expression');
		m.addItem('微笑');
		m.addItem('生气');
		m.addItem('哭泣');
		m.addItem('大笑');
	...
	{
		var m:Menu = new Menu(setBlockArg, 'expression');
		var commandsList:Array = AppDataManager.getInstance().commandsList;
	...
	private function scenesMenu(evt:MouseEvent):void
	{
		var m:Menu = new Menu(setBlockArg, 'scenes');
		var scenesList:Array = AppDataManager.getInstance().sceneList;
		for each(var item:Object in scenesList) {
			m.addItem(item["desc"]);
		}
		showMenu(m);
	}
	
	private function gameMenu(evt:MouseEvent):void
	{
		var m:Menu = new Menu(setBlockArg, 'game');
		m.addItem("抓字母");
		showMenu(m);
	}
	
	private function gestureMenu(evt:MouseEvent):void
	{
